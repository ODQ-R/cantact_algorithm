...
function [dydt] = DEMround2Dnorot(t,y);
    ...
    error('length of y must be four times the length of m')
    end

    if length(rad) ~= length(m)
    error('length of r must be four times the length of m')
    end
...
for i_part = 1:n_part
    ...
    r1 = [y(4*i_part-3);y(4*i_part-1)]; % position of first particle
    rad1 = rad(i_part);
    
    % Particle-Particle Interaction
    for j_part = i_part+1:n_part
        
        r2 = [y(4*j_part-3);y(4*j_part-1)]; % position of second particle
        rad2 = rad(j_part);
